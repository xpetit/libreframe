...
begin
   ...
   Sys.Set_Memory_Monitor;

   Prg.Set_Version (0, 8);
   Log.Set_Display (True);
   Log.Set_Debug (True);
   --Log.Set_Debug_Filter ("LibreFrame.Sql.Get_Column_Length,LibreFrame.Sql.Update");
   --  Log.Set_Debug_Filter ("LibreFrame.Sql.Update, LibreFrame.Sql.Field_Value.MySQL,");

   Tio.Set_Cursor (False);

   Log.New_Line;
   Log.Info ("LibreFrame - API test program");
   Log.Info ("Copyright (C) Sowebio SARL 2020-" & From_Latin_1 (GCT.Image (AC.Clock, "%Y")) & ", according to LGPLv3");
   Log.Info (Prg.Get_Version & " - " & LibreFrame.Get_Version & " - " & LibreFrame.Get_Build);
   Log.New_Line;
   ...
   if String_Option.all /= "" then
      Log.Info ("Switch -s:        : " & From_Latin_1 (String_Option.all));
   end if;
   if Long_Option /= 0 then
      Log.Info ("Switch -l --long= : " & To_String (Long_Option));
   end if;

   if (String_Option.all = "") and (Long_Option = 0) then
      Log.Info ("Try ./test -h or --help");
      Log.Info ("Try ./test -badoption");
      Log.Info ("Try ./test -s=toto --long=123456 -1 -2 (or -12 instead)");
   end if;
   Log.New_Line;
   ...
   Log.Info ("Program name   : " & Prg.Name);
   Log.Info ("User home      : " & Sys.Get_Home);
   Log.Info ("Library version: " & LibreFrame.Get_Version);
   Log.Info ("A time stamp   : " & Prg.Date_Time_Stamp);
   Log.Info ("Debug filter   : " & Get_Debug_Filter);
   Log.New_Line;
   ...
   --  begin
   --
   --     --Log.Info (To_String_Unsigned (AC.Formatting.Day (Day_Previous)));
   --
   --     -- Date
   --     Log.Info ("Date: " & From_ASCII (GCT.Image (AC.Clock, "%Y%m%d")));
   --
   --     -- Day previous
   --     declare
   --        Day_Previous : AC.Time;
   ...
   --        --  Log.Info ("Dow_Name: " & DOW_Name);
   --
   --        --  DOW := ACF.Day_Of_Week (Day_Previous);
   --        --  DOW_Number := ACF.Day_Name'Pos (DOW);
   --        --  Log.Info ("Dow_Number: " & To_String_Unsigned (DOW_Number));
   --
   --        -- Previous day
   --        Day_Previous := AC.Formatting.Value (To_Latin_1 (Prg.Date) & " 00:00:00.00") - 1;
   --        Log.Info ("Day_Previous: " & From_ASCII (GCT.Image (Day_Previous, "%Y%m%d")));
   --
   --        Log.Info ("DOW_Name: " & From_Latin_1 (ACF.Day_Name'Image (ACF.Day_Of_Week (Day_Previous))));
   --        Log.Info ("DOW_Number: " & To_String_Unsigned (Integer (ACF.Day_Name'Pos (ACF.Day_Of_Week (Day_Previous))) + 1 ));
   --
   --        -- First day of week's date
   --        --Day_SOW := Day_Previous - ACF.Day_Name'Pos (ACF.Day_Of_Week (Day_Previous));
   --        Log.Info ("Day_SOW: " & From_ASCII (GCT.Image (Day_Previous - ACF.Day_Name'Pos (ACF.Day_Of_Week (Day_Previous)), "%Y%m%d")));
   --
   --        -- Last day of week's date
   --        --Day_SOW := Day_Previous + (6 - ACF.Day_Name'Pos (ACF.Day_Of_Week (Day_Previous)));
   --        Log.Info ("Day_EOW: " & From_ASCII (GCT.Image (Day_Previous + (6 - ACF.Day_Name'Pos (ACF.Day_Of_Week (Day_Previous))), "%Y%m%d")));
   --
   ...
   --
   --        Day_SOW := Day_Previous - ACF.Day_Name'Pos (ACF.Day_Of_Week (Day_Previous));
   --        Log.Info ("Day_SOW: " & From_ASCII (GCT.Image (Day_SOW, "%Y%m%d")));
   --
   --
   ...
   --        Log.Info ("Day_SOW: " & From_ASCII (GCT.Image (Day_SOW, "%Y%m%d")));
   --
   --        Log.Info ("Month: " & From_ASCII (GCT.Image (AC.Formatting.Value (To_Latin_1 (Prg.Date) & " 00:00:00.00"), "%Y%m")) & "%");
   --
   --
   ...
   if Exception_Test then
      ...
      Log.New_Line;
      Log.Title ("Exception test trigered by a raise exception");
      Log.New_Line;
...
exception
   ...
   --  Runtime errors
   when Error : others =>
      LibreFrame.Exception_Handling (Error);
