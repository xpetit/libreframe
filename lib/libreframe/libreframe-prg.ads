...
package LibreFrame.Prg is
   ...
   function Check_Password (Password : UString) return Boolean;
   --  Check password strength. A password must have at least contains uppercase, lowercase, digit
   --  and one special char among "-" and "_".

   Command : constant UString := From_Latin_1 (Ada.Command_Line.Command_Name);
   ...
   function Date return UString;
   --  Returns current date as YYYY-MM-DD.

   function Date_Not_Reached (DTS : UString) return Boolean;
   --  Returns true if YYYY-MM-DD Date_ISO is still to come.

   function Date_Time_Stamp_Reached (DTS : UString; Zone : Time_UTC_Local := Local) return Boolean;
   --  Returns true if YYYYMMDD:HHMMSS Date_Time_Stamp has passed.

   function Date_Time_Stamp (Zone : Time_UTC_Local := Local) return UString;
   --  Returns current datetime stamp as YYYYMMDD-HHMMSS.

   function Date_Time_Stamp_From_ISO (DTS : UString) return UString;
   --  Returns “YYYYMMDD-HHMMSS” from a date time stamp as “YYYY-MM-DD HH:MM:SS”.

   function Date_Time_Milli_Stamp return UString;
   --  Returns current datetime stamp with trailing milliseconds as: YYYYMMDD-HHMMSS.NNN.
   --  Used in LibreFrame.Log logs

   function Date_Time_Nano_Stamp return UString;
   ...
   function Date_Time_Stamp_To_Date (DTS : UString) return UString;
   --  Returns YYYY-MM-DD from a datetime stamp as YYYYMMDD-HHMMSS.

   function Date_Time_Stamp_To_ISO (DTS : UString) return UString;
   --  Returns “YYYY-MM-DD HH:MM:SS” from a date time stamp as “YYYYMMDD-HHMMSS”.

   function Date_Time_Stamp_From_Local (DTS : UString) return UString;
   --  Returns a date time stamp “YYYYMMDD-HHMMSS” from a local “DD?MM?YYYY HH?MM?SS” (? is any character).

   function Date_Time_Stamp_To_Local (DTS : UString; Sep : UString := "/") return UString;
   --  Returns “DD/MM/YYYY HH:MM:SS” from a date time stamp as “YYYYMMDD-HHMMSS” with a customizable date separator.

   function Date_To_ISO (DTS : UString) return UString;
   --  Returns “YYYY-MM-DD” from a string date as “DD?MM?YYYY” (? is any character).

   function Date_ISO_To_Local (DTS : UString) return UString;
   -- Returns "DD/MM/YYYY" from a string date as "YYYY?MM?DD" (? is any character).

   function Duration_Stamp (Time : Ada.Calendar.Time) return UString;
   ...
   function Duration_Stamp_Time (Time_Seconds : Integer) return UString;
   ...
   function Generate_Password (Characters_Space : UString := LETTERS_LOWER & LETTERS_UPPER & DIGITS_DECIMAL & SPECIAL_CHARACTERS;
                               Password_Length : Positive := 14) return UString;
   ...
   function Is_Handler_Ctrl_C return Boolean;
   --  Get Ctrl-C handling status. If returns True, Ctrl-C is activated and,
   --  when pressed, application is properly finalize. If returns False, Ctrl-C
   --  is inhibited and application continue.

   function Is_User_Not_Root return Boolean;
   --  Returns True if program user's not root.

   function Name return UString;
   --  Program name.

   function Path return UString;
   --  Program path.

   procedure Set_Handler_Ctrl_C (Enable : Boolean);
   ...
   Start_Dir : constant UString := From_Latin_1 (Ada.Directories.Current_Directory);
...
private
   ...
   function Time_Format (Input_To_Format : Integer) return UString;

   Version_Major : Natural := 0;
   Version_Minor : Natural := 0;

   Exit_Status : Natural := 0;
   Ctrl_C_Status : Boolean := True;
