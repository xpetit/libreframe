...
package body LibreFrame.Tio is
   ...
   ----------------------------------------------------------------------------
   function Confirm_Twice (User_Prompt_1 : UString ; User_Prompt_2 : UString) return Boolean is
      Answer : Character;
   ...
   ----------------------------------------------------------------------------
   procedure Bell is
      Sound_File : UString := "/usr/share/sounds/freedesktop/stereo/complete.oga";
      Sound_Player : UString := "/usr/bin/paplay";
   begin
   ...
   ----------------------------------------------------------------------------
   function Get_Line return UString is
      Result : UString := "";
   begin
   ...
   ----------------------------------------------------------------------------
   function Get_Password return UString is
   ...
   ----------------------------------------------------------------------------
   function Is_Ansi return Boolean is
   begin
      return Ansi_Status;
   end Is_Ansi;
   ...
   procedure Put_Line (V : UString) is
   begin
      UTI.Put_Line (V);
   end Put_Line;
   ...
   ----------------------------------------------------------------------------
   procedure Set_Ansi (Enable : Boolean)  is
   begin
      Ansi_Status := Enable;
   end Set_Ansi;
   ...
   procedure Set_ANSI_Color (Attribute :  ANSI_Attribute; Foreground : ANSI_Color := White; Background : ANSI_Color := Black) is
      ...
      Current_Attribute, Current_Color : UString;

      function Put_Color (Color : ANSI_Color) return UString is
         Current_Color : UString;
      begin
         case Color is
         when Black =>
            Current_Color := "0";
   ...
   ----------------------------------------------------------------------------
   procedure Set_Cursor (Enable : Boolean) is
   begin
      if Tio.Is_Ansi then
         if Enable then
   ...
   ----------------------------------------------------------------------------
   function Get_Line (Handle : File) return UString is
   begin
      return From_Latin_1 (ATI.Get_Line (Handle));
   end Get_Line;

   procedure Get_Line (Handle : File; V : out UString) is
   begin
   ...
   procedure Open_Conf (Handle : in out File;
                        ...
                        Permissions : UString := "") is

      Dir_File : constant UString := Fls.Extract_Directory (Name);
   ...
   begin
      ...
      if Fls.Exists (Name) then
         ...
         if Fls.Create_Directory_Tree (Dir_File) then
            ...
            Log.Error (Log_Header & "Can't create directory: " & Dir_File);
         end if;
      end if;
      ...
      if Fls.Exists (Name) and not Is_Empty (Permissions) then
         ...
         if SE_Result /= 0 then
            Log.Error  (Log_Header & "Can't apply permissions to: " & Name);
          end if;
      end if;
   ...
   ----------------------------------------------------------------------------
   procedure Open_Read (Handle : in out File; Name : UString) is
   begin
      ATI.Open (Handle, ATI.In_File, To_Latin_1 (Name));
   end Open_Read;

   ----------------------------------------------------------------------------
   procedure Put (Handle : File; V : UString) is
   begin
   ...
   procedure Put_Line (Handle : File; V : UString) is
   begin
      ATI.Put_Line (Handle, To_Latin_1 (V));
   end Put_Line;

   ----------------------------------------------------------------------------
   function Read_File (File_Name : UString)
                       return UString is

      File_Handle : File;
      Line_Buffer, Result_Buffer : UString := "";
   ...
   ----------------------------------------------------------------------------
   procedure Write_File (File_Name : UString;
                         Content : UString;
                         Permissions : UString := "") is

      Index_Next : Integer := 1;
      String_To_Process : UString := "";
      Dir_File : constant UString := Fls.Extract_Directory (File_Name);
      File_Handle : File;
   ...
   begin
      ...
      if Fls.Create_Directory_Tree (Dir_File) then
         ...
         if Fls.Exists (File_Name) and not Is_Empty (Permissions) then
            ...
            if SE_Result /= 0 then
               Log.Error (Log_Header & "Can't apply permissions to: " & File_Name);
            end if;
         end if;
