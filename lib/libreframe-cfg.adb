...
package body LibreFrame.Cfg is
   ...
   begin
      if Tio.Is_Open (Handle_Read) then
         Tio.Close (Handle_Read);
          ...
          Log.Error ("LibreFrame.Cfg.close > File should be opened before closing: " & Cfg_File_Read);
      end if;
   end Close;

   ----------------------------------------------------------------------------
   procedure Comment (Text : UString) is
      Dummy : Boolean;
   begin
   ...
   ----------------------------------------------------------------------------
   procedure Delete (Section : UString; Parameter : UString) is
   ...
   begin
      ...
      if Cfg_Section > 0 and Cfg_Parameter > 0 then
         ...
         if not Cfg_Write then
             Log.Error ("LibreFrame.Cfg.Delete > Cfg_Write operation failed");
         end if;
      end if;
   end Delete;

   ----------------------------------------------------------------------------
   function Get (Section : UString; Parameter : UString) return UString is
      Result : UString := "";
   begin
      Cfg_Search (Section, Parameter);
      --  Get parameter
   ...
   ----------------------------------------------------------------------------
   function Get_Name return UString is
   begin
   ...
   ----------------------------------------------------------------------------
   function Open (Cfg_File_Read_In : UString := "")
                  return Boolean is
   ...
   begin
      ...
      if not Ada.Directories.Exists (To_Latin_1 (Cfg_File_Read)) then
         ...
         if Tio.Is_Open (Handle_Read) then
            Close;
             ...
             Log.Error (Log_Header & "Can't create file: " & Cfg_File_Read);
         end if;
      end if;
      --  Open in read mode
      ...
      if Tio.Is_Open (Handle_Read) then
         ...
         Log.Error (Log_Header & "Can't read file: " & Cfg_File_Read);
      end if;
      return Result;
   end Open;

   ----------------------------------------------------------------------------
   procedure Set (Section : UString;
                  Parameter : UString;
                  Value : UString;
                  Comment : UString := "") is

      Trailing_Comment : UString := "";
         ...
         --  Create Parameter => tag the section for further just in time write in file
            ...
            if Table_Max >= Cfg_Last + 2 then
               ...
               Cfg_Last := Cfg_Last + 2;
                ...
                Log.Error (Log_Header & "Too large: " & Cfg_File_Read);
                Log.Error (Log_Header & "For further writes to: " & Cfg_File_Write);
            end if;
         end if;

         if not Cfg_Write (Section, Parameter, Value, Trailing_Comment) then
             Log.Error (Log_Header & "Cfg_Write operation failed");
         end if;
      else
          Log.Error (Log_Header & "Invalid character in Section, Parameter or Value in: " & Cfg_File_Write);
      end if;
   end Set;
   ...
   ----------------------------------------------------------------------------
   procedure Cfg_Search (Section : UString; Parameter : UString) is
   ...
   ----------------------------------------------------------------------------
   function Cfg_Write (Section : UString := "";
                       Parameter : UString := "";
                       Value : UString := "";
                       Trailing_Comment : UString := "")
                       return Boolean is
      ...
      if Tio.Is_Open (Handle_Write) then
         ...
         if Tio.Is_Open (Handle_Read) then
            Result := Cfg_Read;
             ...
             Log.Error (Log_Header & "Can't open new: " & Cfg_File_Read);
            Result := False;
         end if;
      else
          Log.Error (Log_Header & "Can't create: " & Cfg_File_Write);
         Result := False;
      end if;
      return Result;
   end Cfg_Write;
   ...
   ----------------------------------------------------------------------------
   function Check_Parameters (Section : UString; Parameter : UString; Value : UString) return Boolean is
      String_To_Test : constant UString := Section & Parameter & Value;
   begin
   ...
   ----------------------------------------------------------------------------
   function Table_Write (Line : UString)
                         return Boolean is
   ...
   begin
      if Cfg_Last = Table_Max then
          Log.Error (Log_Header & "Too large to load: " & Cfg_File_Read);
         Cfg_Last := 0;
         Result := False;
      else
         Cfg_Last := Cfg_Last + 1;
   ...
   ----------------------------------------------------------------------------
   procedure Set_Name (Cfg_File_Read_In : UString) is
   begin
      Cfg_File_Read := Cfg_File_Read_In;
   end Set_Name;
