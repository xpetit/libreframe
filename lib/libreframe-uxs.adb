...
package body LibreFrame.Uxs is
   ...
   ----------------------------------------------------------------------------
   function Char_Count (String_To_Process : UString ; Char_Set_Pattern : UString) return Integer is
   ...
   ---------------------------------------------------------------------------
   function Ends_With (Item : UString; Pattern : ASCII_Character) return Boolean is
   begin
      return 1 <= Item.Length and then From_Unicode (Element (Item, Item.Length)) = From_ASCII (Pattern);
   end Ends_With;

   ---------------------------------------------------------------------------
   function Field_By_Index (String_Input : UString ; Index_Field : Integer ; Field_Delimiter : UString) return UString is
      Index_Count, Index_Next : Integer := 1;
      Result_String : UString := "";
      String_To_Process : UString := String_Input & Field_Delimiter;
   begin
   ...
   ---------------------------------------------------------------------------
   function Field_By_Name (String_Input : UString ; Field_To_Search : UString ; Field_Delimiter : UString) return UString is
      Index_Count : Integer := 1;
      Result_String, Result_Char: UString := "";
      String_To_Process : constant UString := String_Input & Field_Delimiter;
   begin
   ...
   ---------------------------------------------------------------------------
   function Field_Count (String_To_Process : UString ; Field_Delimiter : UString) return Integer is
   begin
   ...
   ----------------------------------------------------------------------------
   procedure Field_Display (String_To_Process : UString;
                            Column_Delimiter: UString ;
                            Row_Delimiter: UString;
                            Custom_Header : UString := "") is
      ...
      Header : constant UString := Custom_Header;
      Max_Width : Natural := 0;
      Current_Column : UString := "";
      type T_Display is array (Integer range <>, Integer range <>) of UString;
   ...
   begin
      Log.Debug (Log_Header & "Columns: " & To_String (Columns), Log_Filter);
      Log.Debug (Log_Header & "Rows: " & To_String (Rows), Log_Filter);
      --  Format
      if Columns > 0 then
         for Index_Columns in 1..Columns loop
            ...
            --Log.Debug (Log_Header & "Max_Width for column: " & Current_Column & ": " & To_String (Max_Width), Log_Filter);
            --  Format header
            if Header.Length > 0 then
   ...
   ----------------------------------------------------------------------------
   function Field_Get_Data (Datas : UString ; Field_Name : UString) return UString is
   ...
   ----------------------------------------------------------------------------
   function Field_Included (String_To_Process : UString ; Items_List  : UString ; Field_Delimiter : UString) return Boolean is
      Items_Count : constant Natural := Field_Count (Items_List, Field_Delimiter);
      Items_Found : Natural := 0;
      Current_Item_Element : UString;
   begin
   ...
   -- Good algo
   function Field_Search (String_Input : UString ; Field_To_Search : UString ; Field_Delimiter : UString) return Boolean is
      Index_Count, Index_Next : Integer := 1;
      Result_String : UString := "";
      String_To_Process : UString := String_Input & Field_Delimiter;
      Result : Boolean := False;
   ...
   ----------------------------------------------------------------------------
   function From_DB_To_Date_String (DB_Value : UString; Separator : UString := "/")
                                    return UString is

      Result : UString := DB_Value;
      Log_Filter : constant UString := From_Latin_1 (GSI.Enclosing_Entity);
      Log_Header : constant UString := Log_Filter & LOG_PROMPT;
   begin
      --  1    6  9
      ...
      if DB_Value.Length >= 10 then
         ...
         Log.Debug (Log_Header & "DB_Value not empty but too short to be processed by 'D' modifier.", Log_Filter);
      end if;
      return Result;
   end From_DB_To_Date_String;

   ----------------------------------------------------------------------------
   function From_DB_To_Money (DB_Value : UString) return Money is
   begin
      return Money (To_Integer (DB_Value)) / 100.00;
   end From_DB_To_Money;

   ----------------------------------------------------------------------------
   function From_DB_To_Money_String (DB_Value : UString) return UString is
   begin
   ...
   ----------------------------------------------------------------------------
   function From_DB_To_Money_String_With_Padding_Sign (DB_Value : UString) return UString is
      Result : UString := From_Latin_1 (Money'Image (Money (To_Integer (DB_Value)) / 100.00));
   begin
   ...
   ----------------------------------------------------------------------------
   function From_Money_To_DB (DB_Value : Money) return UString is
   begin
      return From_Money_To_DB (Trim_Left (From_Latin_1 (Money'Image (DB_Value))));
   end From_Money_To_DB;

   function From_Money_To_DB (DB_Value : UString) return UString is
      Integer_Part, Decimal_Part : UString;
   begin
   ...
   function Is_Numeric (Item : in UString; Signs : UString := "") return Boolean is
      Digits_To_Test : UString := DIGITS_DECIMAL;
   ...
   ----------------------------------------------------------------------------
   function Padding_Left (String_To_Process : UString ; Padding_Character : UString ; Result_Length : Positive) return UString is
   begin
   ...
   ---------------------------------------------------------------------------
   function Replace_Char (String_To_Process : UString ; Char_In : ASCII_Character ; Char_Out : ASCII_Character) return UString is
      Result_String : UString := "";
   ...
   ---------------------------------------------------------------------------
   function Starts_With (Item : UString; Pattern : ASCII_Character) return Boolean is
   begin
      return 1 <= Length (Item) and then From_Unicode (Element (Item, 1)) = From_ASCII (Pattern);
   end Starts_With;

   ---------------------------------------------------------------------------
   function Stript_Chars (String_To_Process : UString ; Char_List : UString) return UString is
      Result_String, Result_Char : UString := "";
   begin
      for I in 1 .. String_To_Process.Length loop
         Result_Char := Slice (String_To_Process, I, I);
   ...
   ---------------------------------------------------------------------------
   function Tail_After_Match (Source : UString; Pattern : UString) return UString is
      Result : UString := "";
   begin
   ...
   function Tail_After_Match (Source : UString; Pattern : ASCII_Character) return UString is
   begin
      return Tail_After_Match (Source, From_ASCII (Pattern));
   end Tail_After_Match;

   ----------------------------------------------------------------------------
   function To_Hex (Byte : Unsigned_8) return UString is
   ...
   function To_Hex (String_In : UString) return UString is
      String_To_Convert : Standard.String := To_Latin_1 (String_In);
      String_Converted : UString := "";
   begin
   ...
   ---------------------------------------------------------------------------
   function To_Hex_From_Val (Input : UString) return UString is
   ...
   ---------------------------------------------------------------------------
   function To_Hex_Control_Codes (String_To_Convert : UString) return UString is
      Current_Char : UString;
      String_Converted : UString := "";
      Ascii_Code : Natural;
   begin
   ...
   ---------------------------------------------------------------------------
   function To_String_From_Hex (String_To_Convert : UString) return UString is
      String_Converted : UString := "";
      Cursor : Natural := 1;
   ...
   ---------------------------------------------------------------------------
   function To_Long_Integer_From_Hex (Hex_Val : UString) return Long_Integer is
      Hex_Value : UString := Trim_Both (Hex_Val);
      Result : Long_Integer := 0;
      Digit_Value : Long_Integer;
      Radix_Offset : Natural := 0;
      Digits_List : constant UString := "0123456789ABCDEF";
   begin
   ...
   ---------------------------------------------------------------------------
   function To_Integer (V : UString) return Integer is
      Result : Integer := 0;
      String_To_Process : UString := Trim_Both (V);
   begin
   ...
   ---------------------------------------------------------------------------
   function To_Long_Long_Integer (V : UString) return Long_Long_Integer is
   ...
   function To_Money (DB_Value : UString) return Money is
   begin
      return Money (To_Integer (DB_Value)) / 100.00;
   end To_Money;

   ---------------------------------------------------------------------------
   function To_String (B : Boolean) return UString is
   begin
      return From_Latin_1 ((if (B) then "True" else "False"));
   end To_String;

   function To_String (I : Integer) return UString is
   begin
      return Trim_Left (From_UTF_8 (I'Image));
   end To_String;

   function To_String (I : Long_Integer) return UString is
   begin
      return Trim_Left (From_UTF_8 (Long_Integer'Image (I))); -- Suppress the space left for positive sign
   end To_String;

   function To_String (I : Long_Long_Integer) return UString is
   begin
      return Trim_Left (From_UTF_8 (Long_Long_Integer'Image (I))); -- Suppress the space left for positive sign
   end To_String;

   function To_String (F : Float) return UString is
   begin
      return Trim_Left (From_UTF_8 (Float'Image (F))); -- Suppress the space left for positive sign
   end To_String;

   function To_String (M : Money) return UString is
   begin
   ...
   function To_String (C : ASCII_Character) return UString is
   begin
      return From_Latin_1 ((1 => C));
   end To_String;

   function To_String_Unsigned (I : Integer) return UString is
   begin
      return From_UTF_8 (I'Image).Delete (1, 1); -- Removes the sign (' ' for plus, '-' for minus)
   end To_String_Unsigned;

   function To_String_Unsigned (I : Unsigned_8) return UString is
   begin
      return From_UTF_8 (I'Image).Delete (1, 1); -- Removes the sign (' ' for plus, '-' for minus)
   end To_String_Unsigned;

   function To_String_Unsigned (I : Unsigned_16) return UString is
   begin
      return From_UTF_8 (I'Image).Delete (1, 1); -- Removes the sign (' ' for plus, '-' for minus)
   end To_String_Unsigned;

   function To_String_Unsigned (I : Long_Integer) return UString is
   begin
      return From_UTF_8 (I'Image).Delete (1, 1); -- Removes the sign (' ' for plus, '-' for minus)
   end To_String_Unsigned;

   ---------------------------------------------------------------------------
   function To_Val (String_To_Convert : UString) return UString is
      String_Converted : UString := "";
      Ascii_Code : Natural;
   begin
   ...
   ---------------------------------------------------------------------------
   function Trim_Both  (Source : UString) return UString is
   begin
      return Trim (Source, AS.Both);
   end Trim_Both;

   ---------------------------------------------------------------------------
   function Trim_Left  (Source : UString) return UString is
   begin
      return Trim (Source, AS.Left);
   end Trim_Left;

   ---------------------------------------------------------------------------
   function Trim_Right (Source : UString) return UString is
   begin
      return Trim (Source, AS.Right);
   end Trim_Right;

   ---------------------------------------------------------------------------
   function Trim_Slashes (Source : UString) return UString is
      Src : UString := Source;
   begin
      --  Specific case
      if Src = "/" then
