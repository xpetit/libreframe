...
package body LibreFrame.Net is

   --  SSH_Temp_Command_Key : String := "-libreframe-net-command-private.key";
   ...
   Temp_Prefix : UString := "-" & To_Lower (LibreFrame.Name) & "-net-";
   ...
   function Api (Api_Provider : Api_Providers;
      ...
      Temp_Output : constant UString := LibreFrame.Get_Tmp_Dir & Prg.Date_Time_Nano_Stamp & Temp_Prefix & "api.output";
      SHA_Handle : GNAT.SHA1.Context;
      Curl_Command : UString := "curl -s ";
      SE_Command : UString;
      SE_Result : Integer := 0;
      SE_Output : UString;

      Result : UString := "";
      Log_Filter : constant UString := From_Latin_1 (GSI.Enclosing_Entity) & "." & From_Latin_1 (Api_Provider'Image);
      Log_Header : constant UString := Log_Filter & LOG_PROMPT;
   begin
      if Api_Provider = Ovh then
         declare
            OVHcloud_Time, OVHcloud_Signature : UString;
            OVHcloud_End_Point : UString := "https://" & Api_End_Point & "/";
            OVHcloud_Method : UString := To_Upper (From_Latin_1 (Api_Method'Image));
         begin
            -- Get OVH time
            ...
            if (SE_Result = 0) then
               ...
               if (SE_Result = 0) then
                  ...
                  Log.Error (Log_Header & "Sending query failed: " & Result);
               end if;
            else
               Log.Error (Log_Header & "Can't get OVH time");
            end if;
         end;
      elsif Api_Provider = Matomo then
------------------------------------------------------------
         declare
            SOWEBIOanalytics_End_Point : UString := "https://" & Api_End_Point & "/";
            SOWEBIOanalytics_Method : UString := To_Upper (From_Latin_1 (Api_Method'Image));
         begin
...
------------------------------------------------------------
         ...
         Log.Error (Log_Header & "Provider not found; " & From_Latin_1 (Api_Provider'Image));
      end if;
   ...
   ---------------------------------------------------------------------------
   function Command (Target : in UString ; Command_In : in UString ; SE_Output : out UString) return Boolean is
      Temp_Key_Name : constant UString := LibreFrame.Get_Tmp_Dir & Prg.Date_Time_Nano_Stamp & SSH_Temp_Command_Key;
      Temp_Output : constant UString := LibreFrame.Get_Tmp_Dir & Prg.Date_Time_Nano_Stamp & Temp_Prefix & "command.output";
      Command_String : UString := "";
      Key_To_Use : UString := "";
      SE_Result : Integer := 0;
   ...
   begin
      ...
      if (SE_Result = 0) then
         if SSH_Message then
            Log.Info ("Remote command " & Command_In & " on " & Target & " successful");
         end if;
         Result := True;
      else
         ...
         if SSH_Message then
             Log.Error (Log_Header & "Command error with: " & Command_String & " on " & Target & " Error: " & To_String (SE_Result));
         end if;
   ...
   ---------------------------------------------------------------------------
   function Command (Target : in UString ; Command_In : in UString) return Boolean is
      Temp_Key_Name : constant UString := LibreFrame.Get_Tmp_Dir & Prg.Date_Time_Nano_Stamp & SSH_Temp_Command_Key;
      Command_String : UString := "";
      Key_To_Use : UString := "";
      Result : Boolean := False;
   ...
   ---------------------------------------------------------------------------
   procedure Command (Target : in UString ; Command_In : in UString) is
      Dummy : Boolean;
   begin
   ...
   ---------------------------------------------------------------------------
   function Copy_File (Target : in UString ; File_Tx : in UString; Directory_Rx : in UString ; Options : in UString := "") return Boolean is
      Temp_Key_Name : constant UString := LibreFrame.Get_Tmp_Dir & Prg.Date_Time_Nano_Stamp & Temp_Prefix & "copy_file-private.key";
      Command_String : UString := "";
      Exec_Error : Integer;
      Key_To_Use : UString := "";
   ...
   begin
      ...
      if (Exec_Error = 0) then
         if  SSH_Message then
            Log.Info ("Remote copy: " & File_Tx & " to " & Directory_Rx & " successful");
         end if;
         Result := True;
      else
         Log.Error (Log_Header & "Copy error with: " & Command_String & " to " & Target & " Error: " & To_String (Exec_Error));
         if Is_Exception then
   ...
   ----------------------------------------------------------------------------
   procedure Copy_File (Target : in UString ; File_Tx : in UString; Directory_Rx : in UString ; Options : in UString := "") is
      Dummy : Boolean;
   begin
   ...
   ---------------------------------------------------------------------------
   function Copy_Rsync (Target : in UString ; Directory_Tx : in UString; Directory_Rx : in UString ; Excludes_Directories : in UString := "") return Boolean is
      Temp_Key_Name : constant UString := LibreFrame.Get_Tmp_Dir & Prg.Date_Time_Nano_Stamp & Temp_Prefix & "copy_rsync-private.key";
      Command_String : UString := "";
      Excludes_String : UString := "";
   ...
   begin
      ...
      if Excludes_Counter > 0 then
         for I in 1 .. Excludes_Counter loop
            Log.Info ("LibreFrame.Net.Copy_Rsync > Excluding: " & Field_By_Index (Excludes_Directories, I, SD));
      ...
      if Net.Directory_Exists (Target, Directory_Rx) then
         Log.Info ("LibreFrame.Net.Rsync > Directory_Rx exists, no need to create it");
      else
         Log.Info ("LibreFrame.Net.Rsync > Directory_Rx don't exists, create it");
         Command (Target, "mkdir --parents " & Directory_Rx);
      end if;
      ...
      if (Exec_Error = 0) then
         if SSH_Message then
            --Log.Info ("Net.Rsync > Copy: ../" & Tail_After_Match (Directory_Tx, '/') & " to " & Directory_Rx & " successful");
            Log.Info ("LibreFrame.Net.Rsync > Copy: " & Directory_Tx & " to " & Directory_Rx & " successful");
         end if;
         Result := True;
      else
         Log.Error (Log_Header & "Rsync error with: " & Command_String & " to " & Target & " Error: " & To_String (Exec_Error));
         if Is_Exception then
            raise Error_Copy_File;
         end if;
      end if;
   ...
   ----------------------------------------------------------------------------
   procedure Copy_Rsync (Target : in UString ; Directory_Tx : in UString; Directory_Rx : in UString ; Excludes_Directories : in UString := "") is
      Dummy : Boolean;
   begin
   ...
   ----------------------------------------------------------------------------
   function Delete_Directory_Tree (Target : in UString ; Dir_Tree : UString) return Boolean is
      Temp_Key_Name : constant UString := LibreFrame.Get_Tmp_Dir & Prg.Date_Time_Nano_Stamp & Temp_Prefix & "delete-directory-tree.key";
      Command_String : UString := "";
      Exec_Error : Integer;
      Key_To_Use : UString := "";
   ...
   ----------------------------------------------------------------------------
   function Delete_File (Target : in UString ; File_To_Delete : in UString) return Boolean is
      Result : Boolean := True;
   begin
   ...
   procedure Delete_File (Target : in UString ; File_To_Delete : in UString) is
      Dummy : Boolean;
   begin
   ...
   ---------------------------------------------------------------------------
   function Directory_Exists (Target : in UString ; Name : UString) return Boolean is
      Temp_Key_Name : constant UString := LibreFrame.Get_Tmp_Dir & Prg.Date_Time_Nano_Stamp & Temp_Prefix & "directory_exists-private.key";
      Command_String : UString := "";
      Exec_Error : Integer;
      Key_To_Use : UString := "";
      Result : Boolean := False;
   ...
   ---------------------------------------------------------------------------
   function File_Exists (Target : in UString ; Name : UString) return Boolean is
      Temp_Key_Name : constant UString := LibreFrame.Get_Tmp_Dir & Prg.Date_Time_Nano_Stamp & Temp_Prefix & "file_exists-private.key";
      Command_String : UString := "";
      Exec_Error : Integer;
      Key_To_Use : UString := "";
      Result : Boolean := False;
   ...
   ---------------------------------------------------------------------------
   function Get_Network_From_Ip (Ip : in UString) return UString is
      Result : UString := "";
   begin
      if Is_Ip_Ok (Ip) then
   ...
   ---------------------------------------------------------------------------
   function Is_Ip_Ok (Ip_In : in UString) return Boolean is
      Ip, Ip_Part : UString := "";
      Ip_Num : Natural;
      Result : Boolean := False;
   begin
   ...
   ---------------------------------------------------------------------------
   function Is_Ping_Ok (Target : in UString) return Boolean is
      SE_Result : Integer := 0;
      SE_Output : UString := "";
   begin
   ...
   ----------------------------------------------------------------------------
   function Is_Root_Directory (Dir_Tree : UString) return Boolean is
   ...
   ----------------------------------------------------------------------------
   function Is_Ssh_Ok (Target : in UString) return Boolean is
      SE_Result : Integer := 0;
   begin
   ...
   ----------------------------------------------------------------------------
   procedure Mount (Target : UString) is

      Temp_Key_Name : constant UString := LibreFrame.Get_Tmp_Dir & Prg.Date_Time_Nano_Stamp & Temp_Prefix & "mount-private.key";
      Mount_Point : constant UString := LibreFrame.Get_Tmp_Dir & Temp_Prefix & "mount/" & Target;
      Exec_Error : Integer;
      Command_String : UString := "";
      Key_To_Use : UString := "";
   ...
   begin
      ...
      if (Exec_Error = 0) then
         ...
         if (Exec_Error = 0) then
            if  SSH_Message then
               Log.Info (Target & " mounted in " & Mount_Point);
            end if;
         else
             Log.Error (Log_Header & "Error mounting: " & Target & " in " & Mount_Point & " Error: " & To_String (Exec_Error));
            if Is_Exception then
               raise Error_Mount;
            end if;
         end if;
      else
          Log.Error (Log_Header & "Error creating local mount point: " & Mount_Point & " Error: " & To_String (Exec_Error));
         if Is_Exception then
            raise Error_Mount;
         end if;
      end if;
   ...
   ----------------------------------------------------------------------------
   function Mount_Remote (Remote_Host : UString ;
                          Target_To_Mount : UString ;
                          Mount_Point : UString ;
                          Mount_Options : in UString := "")
                          return Boolean is
      ...
      if Net.Command (Remote_Host, "mkdir --parents " & Mount_Point) then
         if Net.Command (Remote_Host, "mount " & Mount_Options & " " & Target_To_Mount & " " & Mount_Point) then
            Result := True;
             ...
             Log.Error (Log_Header & "Error mounting: " & Target_To_Mount & " linked to " & Mount_Point & " on " & Remote_Host);
         end if;
      else
          Log.Error (Log_Header & "Error creating mount point: " & Mount_Point & " targetting " & Target_To_Mount & " on " & Remote_Host);
      end if;
      return Result;
   end Mount_Remote;

   procedure Mount_Remote (Remote_Host : UString ; Target_To_Mount : UString ; Mount_Point : UString ; Mount_Options : in UString := "") is
      Dummy : Boolean;
   begin
   ...
   ---------------------------------------------------------------------------
   function Send_Mail (From : UString; To : UString; Subject : UString; Body_Text : UString; Sender : UString := "") return Boolean is
      Message_File : constant UString := LibreFrame.Get_Tmp_Dir & Prg.Date_Time_Nano_Stamp & Temp_Prefix & "send-mail.txt";
      Domain_Name_From : UString := Field_By_Index (From, 2, AR);
      Domain_Name_To : UString := Field_By_Index (To, 2, AR);
      From_With_Sender : UString := From;
      Command_String : UString := "";
      SE_Result : Integer := 0;
      SE_Output : UString;
         ...
         if Index (SE_Output, "mail exchanger =") > 0 then  -- at leat one line domain.com mail exchanger = 10 mx3.mail.ovh.net.
            ...
            if (SE_Result = 0) then
               if Index (SE_Output, "mail exchanger =") > 0 then
                  ...
                  Log.Debug (Log_Header & "Command string: " & Command_String, Log_Filter);
                  Sys.Shell_Execute (Command_String, SE_Result);
                  if (SE_Result = 0) then
                     Result := True;
                  else
                     Log.Error (Log_Header & "Sending failed");
                  end if;
                  Fls.Delete_File (Message_File);
               else
                  Log.Error (Log_Header & "'To domain' has no mail server: " & Domain_Name_To);
               end if;
            else
               Log.Error (Log_Header & "Nslookup failed when testing 'To domain'");
            end if;
         else
            Log.Error (Log_Header & "'From domain' has no mail server: " & Domain_Name_From);
         end if;
      else
         Log.Error (Log_Header & "Nslookup failed when testing 'From domain'");
      end if;
      return Result;
   end Send_Mail;
   ...
   function Send_Sms (Api_Provider : Api_Providers;
      ...
      Sms_Receiver : UString := "";
      Sms_Query : UString := "sms/" & Sms_Account & "/jobs";
   ...
   begin
      if Api_Provider = Ovh then
         ...
         if Sms_Receivers_Count > 0 then
            ...
            --  Log.Info (From_Latin_1 (GJ.Write (Sms_Job)));
   ...
   ---------------------------------------------------------------------------
   procedure Set_Exception (Enable : Boolean := True) is
   begin
      SSH_Exception := Enable;
   end Set_Exception;

   ---------------------------------------------------------------------------
   function Set_Hostname (Target : UString ; Hostname : UString) return Boolean is
      Temp_Key_Name : constant UString := LibreFrame.Get_Tmp_Dir & Prg.Date_Time_Nano_Stamp & Temp_Prefix & "directory_exists-private.key";
      Command_String : UString := "";
      Exec_Error : Integer;
      Key_To_Use : UString := "";
   ...
   ---------------------------------------------------------------------------
   function Set_Key (Key : UString := "")
                     return Boolean is

      Temp_Key_Name : constant UString := LibreFrame.Get_Tmp_Dir & Prg.Date_Time_Nano_Stamp & "-LibreFramereframe-net-set_key-private.key";
      Exec_Error : Integer;
   ...
   begin
      ...
      if (Exec_Error = 0) then
         ...
         Result := True;
          ...
          Log.Error (Log_Header & "Key invalid Error: " & To_String (Exec_Error));
      end if;
      Fls.Delete_File (Temp_Key_Name);
      return Result;
   end Set_Key;
   ...
   ---------------------------------------------------------------------------
   procedure Set_Message (Enable : Boolean := True) is
   begin
      SSH_Message := Enable;
   end Set_Message;

   ---------------------------------------------------------------------------
   procedure Set_Output (Enable : Boolean := True) is
   begin
      SSH_Output := Enable;
   end Set_Output;

   ---------------------------------------------------------------------------
   procedure Set_Password (Password : UString := "") is
      Result : Boolean := False;
   begin
   ...
   ---------------------------------------------------------------------------
   procedure Unmount (Target : UString) is
   ...
   begin
      ...
      if (Exec_Error = 0) then
         Log.Info (Target & " unmounted from " & Mount_Point);
         Sys.Shell_Execute ("rmdir " & Mount_Point & STD_ERR_OUT_REDIRECT, Exec_Error);
         if (Exec_Error = 0) then
            if  SSH_Message then
               Log.Info ("Delete local mount point: " & Mount_Point);
            end if;
         else
             Log.Error (Log_Header & "Error deleting local mount point: " & Mount_Point & " Error: " & To_String (Exec_Error));
            if Is_Exception then
               raise Error_Unmount;
            end if;
         end if;
      else
          Log.Error (Log_Header & "Error unmounting: " & Target & " from " & Mount_Point & " Error: " & To_String (Exec_Error));
         if Is_Exception then
            raise Error_Unmount;
         end if;
      end if;
   ...
   ----------------------------------------------------------------------------
   function Unmount_Remote (Remote_Host : UString;
                            Mount_Point : UString)
                            return Boolean is
      ...
      if Net.Command (Remote_Host, "umount " & Mount_Point) then
         ...
         if not Net.Directory_Exists (Remote_Host, Mount_Point) then
            Result := True;
             ...
             Log.Error (Log_Header & "Mount point: " & Mount_Point & " not deleted on " & Remote_Host);
         end if;
      else
          Log.Error (Log_Header & "Mount point: " & Mount_Point & " not unmounted on " & Remote_Host);
      end if;
      return Result;
   end Unmount_Remote;

   procedure Unmount_Remote (Remote_Host : UString ; Mount_Point : UString) is
      Dummy : Boolean;
   begin
