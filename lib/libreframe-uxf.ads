...
package LibreFrame.Uxf is
   ...
   function Get_Line (Handle : in out File) return UString;
   procedure Get_Line (Handle : in out File; V : in out UString);
   ...
   procedure Open_Read (Handle : in out File; Name : UString; Scheme : Encoding_Scheme := UTF_8);
   --  Open a file in read mode. Maximum file length is one petabyte (1024 TB).

   procedure Put (Handle : File; S : UString) renames UTI.Put;
   --  Write to a file.

   procedure Put_Line (Handle : File; S : UString) renames UTI.Put_Line;
