...
package body LibreFrame.Prg is
   ...
   ----------------------------------------------------------------------------
   function Check_Password (Password : UString) return Boolean is
      Password_Length : Natural := Length (Password);
      Test_Character : UString;
   ...
   begin
      if Password_Length >= PASSWORD_MINIMUM_LENGTH then
         ...
         for I in 1 .. Password_Length loop
            ...
            Log.Debug (Test_Character);
            if Index (LETTERS_LOWER, Test_Character) > 0 then
               Log.Debug ("LibreFrame.Prg.Check_Password > Password has lower case character");
               Lower_Ok := True;
            elsif Index (LETTERS_UPPER, Test_Character) > 0 then
               Log.Debug ("LibreFrame.Prg.Check_Password > Password has upper case character");
               Upper_Ok := True;
            elsif Index (DIGITS_DECIMAL, Test_Character) > 0 then
               Log.Debug ("LibreFrame.Prg.Check_Password > Password has numeric character");
               Numeric_Ok := True;
            elsif Index (SPECIAL_CHARACTERS, Test_Character) > 0 then
               Log.Debug ("LibreFrame.Prg.Check_Password > Password has special character");
               Special_Ok := True;
            end if;
         end loop;
      else
         Log.Debug ("LibreFrame.Prg.Check_Password > Password " & Password & " too short");
      end if;
   ...
   ----------------------------------------------------------------------------
   function Date return UString is
   begin
      return Date_Time_Stamp_To_Date (Date_Time_Stamp);
   end Date;

   ----------------------------------------------------------------------------
   function Date_Not_Reached (DTS : UString) return Boolean is
      Result : Integer := 0;
   begin
      --  0  0 00 01 11 11 11
      --  1  4 67 90 23 56 89
      ...
      if DTS.Length >= 10 and Slice (DTS, 5, 5) = "-" and Slice (DTS, 8, 8) = "-" then
         ...
         begin
            Log.Error (Log_Header & "Input string not compliant: " & DTS);
         end;
      end if;
      return (Result > 0);
   ...
   ----------------------------------------------------------------------------
   function Date_Time_Stamp_Reached (DTS : UString ; Zone : Time_UTC_Local := Local) return Boolean is
      ...
      DTS_UTC, DTS_Local, CLK_UTC, CLK_Local : UString;
   begin
      ...
      if DTS.Length = 15 and Slice (DTS, 9, 9) = "-" then
         ...
         Log.Debug ("DTS:       " & DTS);
         Log.Debug ("DTS_UTC:   " & DTS_UTC);
         Log.Debug ("DTS_Local: " & DTS_Local);
         Log.Debug ("CLK_UTC:   " & CLK_UTC);
         Log.Debug ("CLK_Local: " & CLK_Local);

         if Zone = UTC then
            DTS_UTC_Numeric := To_Long_Long_Integer (Replace (DTS_UTC, "-", ""));
            CLK_UTC_Numeric := To_Long_Long_Integer (Replace (CLK_UTC, "-", ""));
            Log.Debug ("DTS_UTC_Numeric: " & To_String (DTS_UTC_Numeric));
            Log.Debug ("DTS_CLK_Numeric: " & To_String (CLK_UTC_Numeric));
            Log.Debug ((DTS_UTC_Numeric <= CLK_UTC_Numeric));
         else
            DTS_Local_Numeric := To_Long_Long_Integer (Replace (DTS_Local, "-", ""));
            CLK_Local_Numeric := To_Long_Long_Integer (Replace (CLK_Local, "-", ""));
            Log.Debug ("DTS_LOC_Numeric: " & To_String (DTS_Local_Numeric));
            Log.Debug ("DTS_CLK_Numeric: " & To_String (CLK_Local_Numeric));
            Log.Debug ((DTS_Local_Numeric <= CLK_Local_Numeric));
         end if;
   ...
   ----------------------------------------------------------------------------
   function Date_Time_Stamp (Zone : Time_UTC_Local := Local) return UString is
      UTC_Offset : ACT.Time_Offset := ACT.UTC_Time_Offset;
   begin
      if Zone = UTC then
   ...
   ----------------------------------------------------------------------------
   function Date_Time_Stamp_From_ISO (DTS : UString) return UString is
      Result : UString := "";
   begin
      --  0  0 00 01 11 11 11
      --  1  4 67 90 23 56 89
      ...
      if DTS.Length >= 19 then
         ...
         declare
            ...
            Log_Header : constant UString := Log_Filter & LOG_PROMPT;
         begin
            Log.Error (Log_Header & "Input string too short: " & DTS);
         end;
      end if;
      return Result;
   ...
   ----------------------------------------------------------------------------
   function Date_Time_Milli_Stamp return UString is
   begin
      return From_ASCII (GCT.Image (AC.Clock, "%Y%m%d-%H%M%S.%i"));
   end Date_Time_Milli_Stamp;

   ----------------------------------------------------------------------------
   function Date_Time_Nano_Stamp return UString is
   begin
      return From_ASCII (GCT.Image (AC.Clock, "%Y%m%d-%H%M%S.%o"));
   end Date_Time_Nano_Stamp;

   ----------------------------------------------------------------------------
   function Date_Time_Stamp_To_Date (DTS : UString) return UString is
      Result : UString := "";
   begin
      --  0  00000 111111
      --  1  45678 012345
   ...
   ----------------------------------------------------------------------------
   function Date_Time_Stamp_To_ISO (DTS : UString) return UString is
      Result : UString := "";
   begin
      --  0  00000 111111
      --  1  45678 012345
   ...
   ----------------------------------------------------------------------------
   function Date_Time_Stamp_From_Local (DTS : UString) return UString is
      Result : UString := "";
   begin
      --  00 00 0  1 11 11 11
      --  12 45 7  0 23 56 89
   ...
   ----------------------------------------------------------------------------
   function Date_Time_Stamp_To_Local (DTS : UString; Sep : UString := "/") return UString is
      Result : UString := "";
   begin
      --  0  00000 111111
      --  1  45678 012345
   ...
   ----------------------------------------------------------------------------
   function Date_To_ISO (DTS : UString) return UString is
      Result : UString := "";
   begin
      --  00 00 0001
      --  12 45 7890
   ...
   ----------------------------------------------------------------------------
   function Date_ISO_To_Local (DTS : UString) return UString is
         Result : UString := "";
      begin
      if DTS.Length >= 10 then
   ...
   ----------------------------------------------------------------------------
   function Duration_Stamp (Time : Ada.Calendar.Time) return UString is
      Day_Secs : Natural;
   begin
   ...
   ----------------------------------------------------------------------------
   function Duration_Stamp_Time (Time_Seconds : Integer) return UString is
   begin
      --  Unlimited hours counter for long uptimes
   ...
   ----------------------------------------------------------------------------
   function Generate_Password (Characters_Space : UString := LETTERS_LOWER & LETTERS_UPPER & DIGITS_DECIMAL & SPECIAL_CHARACTERS;
                               Password_Length : Positive := 14) return UString is
   ...
   ----------------------------------------------------------------------------
   function Is_Handler_Ctrl_C return Boolean is
   begin
      return Ctrl_C_Status;
   end Is_Handler_Ctrl_C;
   ...
   ----------------------------------------------------------------------------
   function Name return UString is
      Result : UString := Tail_After_Match (Command, "/");
   begin
   ...
   ----------------------------------------------------------------------------
   function Path return UString is
   ...
   --  begin
   --     accept Start;
   --     Log.Info ("Tracking armed for " & Trim_Left (Field_By_Index (Image (Delay_Value), 1, ".")) & "s cycles");
   --     loop
   --        delay Delay_Value;
   --        Prg.Tracking_Users;
   --     end loop;
   ...
   ----------------------------------------------------------------------------
   function Time_Format (Input_To_Format : Integer) return UString is
      Two_Digits_Output : UString;
   begin
   ...
   --  begin
   --     Log.Debug ("User tracking");
   --
   --     --  for C in Databases.Iterate loop
   --     --     if Databases(C).Brand = MySQL then
   --     --        Databases(C).DBM.Execute_Query ("SELECT 1");
   --     --        Log.Info ("Ping on " & From_Latin_1 (Databases(C).Brand'Image) & " database: " & Databases(C).Name);
