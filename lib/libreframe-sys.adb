...
--  |_ | |_) | (/_ |  | (_| | | | (/_
--
--  @file      libreframe-sys.adb
--  @copyright See authors list below and README.md file
--  @licence   LGPL v3
--  @encoding  UTF-8
-------------------------------------------------------------------------------
--  @summary
--  LibreFrame framework - System package
--
--  @description
...
package body LibreFrame.Sys is
   ...
   ----------------------------------------------------------------------------
   function Command_Path (Command_Name : UString) return UString is
      Exec_Error : Integer;
      Find_Command : constant UString := "which " & Command_Name;
      Exec_Output : UString;
   begin
   ...
   ----------------------------------------------------------------------------
   function CRC16_Compute (String_Hex : UString) return Unsigned_16 is
   ...
   ---------------------------------------------------------------------------
   function Get_Alloc_Ada return UString is
   ...
   function Get_Alloc_All return UString is
   ...
   ---------------------------------------------------------------------------
   function Get_Env (Name : UString) return UString is
   begin
   ...
   ---------------------------------------------------------------------------
   function Get_Home return UString is
   begin
      return Get_Env ("HOME");
   end Get_Home;
  ...
  ---------------------------------------------------------------------------
   function Get_System_Name return UString is
      System_Name : UString := Tio.Read_File ("/etc/issue.net");
   begin
   ...
   function Get_System_Version return UString is
      System_Name : constant UString := Tio.Read_File ("/etc/issue.net");
      System_Version : UString := "";
   begin
      if (Index (System_Name, "Debian") > 0) then
   ...
   ---------------------------------------------------------------------------
   function Is_Package (Package_Name : UString; Host_Name : UString := "") return Boolean is
      Exec_Error : Integer;
      Check_Command : constant UString := "dpkg -s " & Package_Name;
      Exec_Output : UString;
      Exec_Output_Install_Ok : constant UString := "Status: install ok installed";
   ...
   begin
      ...
      Log.Debug (Log_Header & " & Is_Install Exec_Output: " & Exec_Output, Log_Filter);
      return (Index (Exec_Output, Exec_Output_Install_Ok) > 0);
   end Is_Package;

   ---------------------------------------------------------------------------
   function Is_Packages (Packages_List : UString; Host_Name : UString := "") return Boolean is
      Packages_String : constant UString := Packages_List;
      ...
      Log_Header : constant UString := Log_Filter & LOG_PROMPT;
   begin
      --Log.Info ("Check installed packages.");
      ...
      if (Packages_Count > 0) then
         ...
         for I in 1 .. Packages_Count loop
            if not Is_Package (Field_By_Index (Packages_String, I, VD), Host_Name) then
               Result := False;
               Log.Error (Log_Header & "Missing packkage: " & Field_By_Index (Packages_String, I, VD));
            end if;
         end loop;
      end if;
      return Result;
   ...
   ---------------------------------------------------------------------------
   function Install_Packages (Packages_List : UString; Host_Name : UString := "") return Boolean is
      Packages_String : constant UString := Packages_List;
      Packages_Count : Natural;
      Result : Boolean := True;
   begin
      Log.Info ("LibreFrame.Sys.Install_Packages > Check packages to install.");
      Packages_Count := Field_Count (Packages_String, VD);
      if (Packages_Count > 0) then
         -- Install packages
         for I in 1 .. Packages_Count loop
   ...
   ---------------------------------------------------------------------------
   function Is_Command (Command : UString) return Boolean is
      ...
      Command_Processed : constant UString := Field_By_Index (Trim_Both (Command), 1, SP);
   begin
   ...
   ---------------------------------------------------------------------------
   procedure Set_Env (Name : UString; Value : UString) is
   begin
      AEV.Set (To_ASCII (Name), To_ASCII (Value));
   end Set_Env;

   ---------------------------------------------------------------------------
   procedure Set_Memory_Monitor (Enable : Boolean := True) is
   begin
      GNATCOLL.Memory.Configure (Activate_Monitor => Enable);
   end Set_Memory_Monitor;

   ---------------------------------------------------------------------------
   --  https://rosettacode.org/wiki/Execute_a_system_command#Ada
   procedure Shell_Execute (Command : UString; Result : out Integer) is
      function Sys (Arg : Interfaces.C.char_array) return Integer;
      pragma Import (C, Sys, "system");
   begin
   ...
   procedure Shell_Execute (Command : UString) is
      Dummy : Integer := 0;
   begin
      Shell_Execute (Command, Dummy);
   end Shell_Execute;

   procedure Shell_Execute (Command : UString; Result : out Integer; Output : out UString) is
   begin
   ...
   function To_Unsigned_16 (String_In : UString) return Unsigned_16 is
   ...
   ---------------------------------------------------------------------------
   function To_Unsigned_8 (String_In : UString) return Unsigned_8 is
   ...
   ---------------------------------------------------------------------------
   function Install_Package (Package_Name : UString;
                             Host_Name : UString)
                             return Boolean is
      ...
      Installer : UString := "apt-get";
      Package_Name_Trimmed : constant UString := Trim_Both (Package_Name);
   ...
   begin
      ...
      if not Is_Package (Package_Name_Trimmed, Host_Name) then
         if Is_Empty (Host_Name) then
            Log.Info (Log_Header & "Local install of: " & Package_Name_Trimmed);
            if Prg.Is_User_Not_Root then
               Installer := "sudo " & Installer;
            end if;
            Log.Debug (Log_Header & "Command: " & Installer & " install -y " &
                         Package_Name_Trimmed & STD_ERR_OUT_REDIRECT, Log_Filter);
            Sys.Shell_Execute (Installer & " install -y " & Package_Name_Trimmed & STD_ERR_OUT_REDIRECT, Exec_Error);
         else
            Log.Info (Log_Header & "Remote install of: " & Package_Name_Trimmed);
         ...
         if Exec_Error = 0 then
            Log.Info (Log_Header & Package_Name_Trimmed & " installed successfully.");
         else
            Log.Error (Log_Header & "Exec error installing: " & Package_Name_Trimmed & " Error code: " & To_String (Exec_Error));
            Result := False;
         end if;
      else
         Log.Info (Log_Header & Package_Name_Trimmed & " already installed.");
      end if;
      return Result;
   end Install_Package;

   ---------------------------------------------------------------------------
   function Purge_Package (Package_Name : UString;
                           Host_Name : UString)
                           return Boolean is
   ...
   begin
      if Is_Package (Package_Name, Host_Name) then
         Log.Info ("LibreFrame.Sys.Purge_Package > Removing " & Package_Name);
         if Is_Empty (Host_Name) then
            if Prg.Is_User_Not_Root then
               Installer := "sudo " & Installer;
            end if;
         ...
         if Exec_Error = 0 then
            ...
            Log.Error (Log_Header & "Exec error purging: " & Package_Name);
            Result := False;
         end if;
      else
         Log.Info (Package_Name & " not installed.");
      end if;
      return Result;
   end Purge_Package;

   function Purge_Packages (Packages_List : UString; Host_Name : UString := "") return Boolean is
      Packages_String : constant UString := Packages_List;
      Packages_Count : Natural;
      Result : Boolean := True;
   begin
      Log.Info ("Check packages to purge.");
      Packages_Count := Field_Count (Packages_String, VD);
      if (Packages_Count > 0) then
         -- Purge package
         for I in 1 .. Packages_Count loop
   ...
   ---------------------------------------------------------------------------
   procedure Shell_Execute_Output (Command : UString; Result : out Integer; Output : out UString) is
